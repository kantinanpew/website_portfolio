import login from "../assets/teaflow/loginpage.jpeg";
import photog_website from "../assets/photoweb.png";

export const HERO_CONTENT = `I am currently a junior at Dong Hwa University in Taiwan, majoring in Computer Science and Information Engineering. Through dedicated efforts, I have maintained a cumulative GPA of 4.04 ranking 3rd in my class. I’m particularly interested in front-end development`;

export const ABOUT_TEXT = `I have also developed some front-end projects where I focused on creating intuitive and responsive designs using HTML, CSS, React and React Native.  My friends and I  just won competition in the Meet Pacific Hualien Start-up Festival 2024, We created a mobile app(Tea flow) to make farm management easier for tea farmers. With a built-in map it helps farmers navigate their fields and monitor crops remotely using drones. The app provides real-time updates on crop yields and uses AI to spot pests and diseases early. From planting to pest control and harvesting, our app supports every step of the tea farming process.
Recently, I am working on a project to build a computer vision system for interpreting American Sign Language which deepened my knowledge of machine learning and user-focused design. I am very passionate about learning new things and skills.`;
export const EXPERIENCES = [
  {
    year: "--",
    role: "--",
    company: "--",
    description: `--`,
    technologies: ["--"],
  },
];
export const EDUCATION = [
  {
    year: "2006-2022",
    school: "Roong Aroon school",
    field: "Mathematics-Science Programme",
    gpa: "3.37 / 4",
  },
  {
    year: "2020",
    school: "AFS exchange program",
    field: "Córdoba, Argentina",
    gpa: " -",
  },
  {
    year: "2022-present",
    school: "National Dong Hwa Univeristy",
    field: "Computer Science and information Engineering",
    gpa: " 4.04 / 4.5",
  },
];

export const PROJECTS = [
  {
    title: "Tea flow",
    image: login,
    description:
      "This mobile application is a comprehensive solution designed to streamline the management of tea farms. It offers a range of features including real-time data management, and field monitoring. Developed using React Native and Firebase, the app provides cross-platform functionality to assist farm managers in making informed decisions based on live data.",
    technologies: ["React Native"],
  },
  {
    title: "Photography website",
    image: photog_website,
    description:
      "This website showcases a curated collection of my photography work, featuring a variety of images that reflect my artistic style and creative vision.",
    technologies: ["React ", "Framemotion"],
  },
];

export const CONTACT = {
  address:
    " 974, Hualien County, Shoufeng Township, Section 2, Daxue Rd, 1-68號、1-69號1-70號 ",
  phoneNo: "+886 911333587 ",
  email: "kantinan.pew@gmail.com",
};
